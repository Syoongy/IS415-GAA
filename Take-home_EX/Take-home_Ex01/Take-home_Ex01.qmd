...
We now want to combine both our geospatial datasets and also convert multiline strings into linestrings through `st_intersection()` and `st_line_merge()` from `sf`
```{r}
#| eval: false
myanmar_merged <- st_intersection(osm_roads_sf, madm1_sf)

# Separate LINESTRING and MULTILINESTRING geometries
linestrings <- myanmar_merged %>%
  filter(st_geometry_type(.) == "LINESTRING")

multilinestrings <- myanmar_merged %>%
  filter(st_geometry_type(.) == "MULTILINESTRING")

# Apply st_linemerge() to MULTILINESTRING geometries
merged_multilinestrings <- multilinestrings %>%
  st_line_merge()

# Combine the LINESTRING geometries with the merged MULTILINESTRING geometries
myanmar_merged <- rbind(linestrings, merged_multilinestrings)
```
...
```{r}
myanmar_merged <- read_rds("data/rds/myanmar_merged.rds")
```
```{r}
tm_shape(madm1_sf) +
  tm_polygons() +
